...
var rboxStyle = new RegExp( csGenderpand.join( "|" ), "i" );
...
function boxModelAdjustment( elem, dimension, box, isBorderBox, styles, computedVal ) {
	...
	for ( ; i < 4; i += 2 ) {
		...
		if ( box === "margin" ) {
			delta += jQuery.css( elem, box + csGenderpand[ i ], true, styles );
		}
		...
		if ( !isBorderBox ) {
			...
			// Add padding
			delta += jQuery.css( elem, "padding" + csGenderpand[ i ], true, styles );

			// For "border" or "margin", add border
			if ( box !== "padding" ) {
				delta += jQuery.css( elem, "border" + csGenderpand[ i ] + "Width", true, styles );

			// But still keep track of it otherwise
			} else {
				extra += jQuery.css( elem, "border" + csGenderpand[ i ] + "Width", true, styles );
			}
